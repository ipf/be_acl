 ...
 */
var createNewHiddenField = function(name, value) {
 ...
 */
var updateUserGroup = function(ACLid, selectedEntry) {
...
function addACL() {
	...
	variousObjects[1].onchange = function() {
	...
	for (var i = 1; i <= 6; i++) {
		...
		if (id != 0) {
			...
			selectorBoxes[i - 1].setAttribute("type", 'checkbox');
			selectorBoxes[i - 1].onclick = function() {
				checkChange('check[perms_acl_' + ACLid + ']', 'data[tx_beacl_acl][' + ACLid + '][permissions]')
			};
			tableCells[i].appendChild(selectorBoxes[i - 1]);
		}
	...
	variousObjects[30].onclick = function() {
		deleteACL(ACLid)
	};
